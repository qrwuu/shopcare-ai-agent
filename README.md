...
- [完整 PRD](docs/ShopCare_PRD.md)：需求细节、验收、异常、安全、指标与评测。
- [PRD 精简版](docs/ShopCare_PRD_精简版.md)：5–10 分钟了解问题、策略、MVP、流程与评测。
- [产品迭代与关键决策](docs/PRODUCT_DECISIONS.md)：核心取舍、验证信号与下一步方向。

## 产品设计

店小服以“对话承接、事实核验、受控执行、状态回流”为主线；完整需求与关键取舍见产品资料。

## 核心亮点

- **一个服务闭环：** 消费者从对话完成咨询、申请与进度跟进；审核员在完整上下文中接续复杂申请。
- **关键动作受控：** 取消订单、修改地址、物流拦截和售后提交先说明影响、再确认、再由服务端校验。
- **状态持续可见：** 对话、售后记录、审核任务和通知围绕同一申请同步，刷新或重新登录后恢复进度。
