...
| 版本 | 日期 | 修订说明 |
| --- | --- | --- |
| V1.0 | 2025-09-01 | 建立项目目标、服务闭环、功能边界、风险控制与验收基线。 |
...
本项目通过现有电商服务流程拆解、典型多轮任务走查和项目能力验证来界定需求，不将演示数据或离线测试结果等同于真实经营指标。

| 观察维度 | 服务问题 | 店小服的产品回应 |
| --- | --- | --- |
| 咨询与执行混杂 | 用户询问规则时不应被强制选择订单；改变订单时不能只靠文本判断 | 将政策解释与订单操作拆分：前者可直接回答，后者必须绑定订单并校验 |
| 短回复与上下文丢失 | “黑色”“确认”“算了”等表达脱离上文无法判断 | 持久化当前商品、订单、规格、地址草稿、待确认动作和售后阶段 |
| 售后过程不可见 | 用户提交申请后不知道是否缺材料、谁在处理、下一步是什么 | 用统一售后记录驱动对话卡片、记录页、审核任务与通知 |
| 高风险动作失控 | 退款、改址、取消和拦截会影响用户权益与资金 | 使用身份、归属、状态、金额和确认门禁；写操作由业务服务执行 |
| 人工介入上下文断裂 | 审核员需跨页面重新拼接信息，用户需反复解释 | 以审核任务聚合用户、订单、对话、附件、规则核验与操作历史 |

由此形成四项产品原则：

1. 先确认事实，再生成答复。
2. 先说明影响，再触发关键动作。
3. 状态是服务闭环的共同真值，不由单一页面自行维护。
4. 自动化无法安全判断时，保留上下文转人工，而非给出臆测结论。
...
| 边界项 | 说明 |
| --- | --- |
| 真实交易与支付 | 使用隔离的演示订单、商品、物流和退款状态，不连接真实支付渠道或商家后台。 |
| 自主资金决策 | Agent 不自行决定退款金额、赔付额度或突破平台政策。 |
| 全领域通用问答 | 非电商服务范围的问题会被礼貌分流，不作为知识百科产品建设。 |
| 复杂商家经营能力 | 不覆盖商家侧营销、定价、库存经营和多店铺运营后台。 |
...
![店小服消费者端首页](<docs/images/屏幕截图 2026-07-20 191746.png>)
...
![消费者账号登录页](<docs/images/屏幕截图 2026-07-20 190820.png>)
...
![商品推荐结果与会话历史](<docs/images/屏幕截图 2026-07-20 191036.png>)

图 3　商品推荐结果。Agent 根据防晒需求给出多个可选商品卡片，消费者可继续查看详情或选择规格。

![商品详情与规格选择](<docs/images/屏幕截图 2026-07-20 191219.png>)
...
![我的订单列表](<docs/images/屏幕截图 2026-07-20 191702.png>)

图 5　我的订单列表。消费者可浏览本人订单及其当前状态，并从对应订单进入咨询。

![修改地址信息收集](<docs/images/屏幕截图 2026-07-20 191339.png>)

图 6　修改地址的信息收集。系统先展示已绑定订单和原收货地址，再要求用户补充完整的新地址。

![修改地址确认完成](<docs/images/屏幕截图 2026-07-20 191349.png>)

图 7　修改地址的确认与执行结果。用户明确确认后，系统返回更新后的收货地址；该结果由订单服务写入，而非模型自行生成。
...
![退款政策解释与订单选择](<docs/images/屏幕截图 2026-07-20 191532.png>)

图 8　退款政策解释与订单选择。系统先回答七天无理由规则；当用户转为具体退款诉求时，再引导其定位要处理的订单。

![退款申请转人工审核](<docs/images/屏幕截图 2026-07-20 191559.png>)
...
![售后审核队列与任务概览](<docs/images/屏幕截图 2026-07-20 191804.png>)

图 10　售后审核工作台。左侧队列按任务状态和风险等级呈现待办；详情区域汇集用户、订单、商品与 Agent 核验信息。

![审核详情中的对话与操作](<docs/images/屏幕截图 2026-07-20 191829.png>)
...
| 序号 | 截图名称 | 建议位置 | 当前状态 |
| --- | --- | --- | --- |
| 1 | 消费者端首页 | 2.3 后 | 已补充，图 1 |
| 2 | 消费者账号登录 | 4.1 后 | 已补充，图 2 |
| 3 | 商品推荐与规格选择 | 4.2 后 | 已补充，图 3—4 |
| 4 | 订单列表与修改地址确认 | 4.3 后 | 已补充，图 5—7 |
| 5 | 退款申请与人工审核状态 | 4.4 后 | 已补充，图 8—9 |
| 6 | 审核队列与审核详情 | 4.5 后 | 已补充，图 10—11 |
