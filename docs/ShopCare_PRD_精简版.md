...
配套阅读：[完整 PRD](./ShopCare_PRD.md) · [产品迭代与关键决策](./PRODUCT_DECISIONS.md)。
...
| 目标用户 | 核心目标 | 优先级 |
| --- | --- | --- |
| 电商消费者 | 用自然语言解决商品、订单、物流和售后问题，并持续看到下一步。 | MVP 优先用户 |
| 售后审核员 | 在订单、对话和凭证齐备的上下文中处理复杂申请。 | MVP 优先用户 |
| 客服/运营负责人 | 维护规则、复盘审核原因和服务质量。 | 后续运营角色 |
...
| 设计原则 | 设计落点 |
| --- | --- |
| 真实/可信执行 | 订单、金额、物流、资格和状态来自受控服务，模型不直接写入。 |
| 关键歧义确认 | 对订单对象、地址字段、售后路径和关键动作进行最小必要确认。 |
| 先完成最短闭环，再扩展能力 | MVP 优先完成订单锚定、确认、申请、审核与状态回流。 |
| 自动执行优先 | 事实充分、规则明确且不涉及待确认写入时，直接完成查询或状态推进。 |
| 过程可追溯 | 售后记录、时间线、通知和审核任务围绕同一申请同步。 |
