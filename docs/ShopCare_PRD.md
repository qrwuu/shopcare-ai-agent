...
配套阅读：[PRD 精简版](./ShopCare_PRD_精简版.md) · [产品迭代与关键决策](./PRODUCT_DECISIONS.md)。
...
| V1.2 | 2026-02-28 | 收敛 MVP，补充需求依据、验收、安全、评测与决策资料入口。 |
...
| 客服/运营负责人 | 维护服务规则、复盘审核原因和质量问题。 | 后续运营角色；规则配置与运营后台纳入 P2。 |
...
| 场景 | 主要用户 | 服务结果 |
| --- | --- | --- |
| 商品与政策咨询 | 消费者 | 商品推荐、规格/库存追问，或规则条件与办理建议。 |
| 订单与物流服务 | 消费者 | 本人订单、物流事实、催发货或履约操作入口。 |
| 售后申请与进度 | 消费者 | 售后申请、凭证、退货单号、时间线和通知。 |
| 人工审核协同 | 审核员 | 聚合订单、对话、附件与历史，完成通过、拒绝或补材料。 |
...
| 原则 | 产品要求 |
| --- | --- |
| 真实/可信执行 | 订单、金额、物流、资格与状态由业务服务返回；模型基于已核验事实解释和引导。 |
| 关键歧义确认 | 对订单对象、地址字段、售后路径和高影响动作等会改变处理结果的信息发起最小必要确认。 |
| 先完成最短闭环，再扩展能力 | MVP 优先跑通订单锚定、确认、售后申请、审核和状态回流；真实系统接入与运营能力在后续版本推进。 |
| 自动执行优先 | 事实充分、规则明确且不涉及待确认写入时，直接完成查询、说明或下一步状态推进。 |
| 过程可追溯 | 用户可查看订单、申请状态、时间线与审核说明；审核员可查看订单、对话、附件和历史。 |
| 失败显式反馈 | 信息不足、规则不支持、上传失败、网络或服务异常时说明原因、已完成部分与恢复方式。 |
...
| 决策 | 产品取舍 |
| --- | --- |
| 对话优先，订单锚定 | 用对话降低表达门槛；具体查询与写入绑定本人订单，必要时澄清对象。 |
| 确认后执行 | 取消、改址、拦截和售后提交以明确确认和服务端二次校验保护。 |
| 售后记录为真值 | 用申请、时间线、通知与审核日志承接跨页面状态，而非只依赖聊天文本。 |

[产品迭代与关键决策](./PRODUCT_DECISIONS.md) 记录了用户问题、方案取舍和验证信号。
...
评测直接驱动真实 HTTP/SSE 聊天链路，并在隔离 PostgreSQL、Redis、独立用户和订单数据中运行。每轮保存输入输出、执行路由、状态前后快照、工具轨迹、确认标记、耗时和人工复核队列。

| 数据集 | 范围 | 用途 |
| --- | --- | --- |
| `release-v1` 冻结回归集 | 96 个会话 | 稳定覆盖商品、规则、订单、物流、售后和上下文回归。 |
| `release-v1` 密封留出集 | 24 个会话 | 独立检验未参与策略调整的服务场景。 |
| `release-v1` 红队集 | 12 个会话 | 覆盖跨用户访问、孤立确认、提示注入、金额与范围门禁。 |

策略对比固定模型、数据和工具条件，围绕短回复承接、否定与诉求切换、多意图关键动作排序、最小必要澄清、孤立确认处理和基于事实的答复进行调整。

评测将意图不一致、应澄清未澄清、回复缺少必要信息、禁用表述、确认前状态变化、确认后未发生预期状态变化、订单错配、工具未调用或调用失败、人工交接缺失、路由错误和调用次数超限归为失败类型。复盘按意图理解、上下文承接、规则、工具契约、状态写入与答复文案归因；调整方向是补充失败用例、优化策略或服务校验，并在冻结、留出和红队集复测。

### 11.3 保留的发布结果

`release-v1-final-a-r4` 共运行 **132 个会话、198 轮中文对话**。
